...
class Exporter  {
        ...
        .then((client) => {
            if (client) {
                ...
                for (let i = 0; i < process.CONFIG.jwt.token.length; i++) {
                    input.push(process.CONFIG.jwt.token[i])
                    input.push(0)
                }
                if (input.length > 1)
                    process.exporter.setMultiKeyValuesIntoRedis('token', input)
            }
        })
        .catch();
    }
    ...
    authenticateToken(req, res, next) {
        ...
        if (typeof bearerHeader != 'undefined') {
            ...
            jwt.verify(bearerToken, process.CONFIG.jwt.token[0], (err, data) => {
                if (err)
                    res.status(404)
                else {
    ...
    deleteMultiKeyValuesIntoRedis(key, value, redisClient = this.redisClient) {
        return new Promise((resolve, reject) => {
            if (!redisClient || typeof redisClient == 'undefined' || !key || typeof key == 'undefined' || !value || typeof value == 'undefined' || key == '' || value == '')
                reject(false);
